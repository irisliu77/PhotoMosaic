...
for x=1:pheight:h
    for y=1:pwidth:w
        ...
        for i=1:10
            min_list(i) = (average(1,i)-ave_r)^2 + (average(2,i)-ave_b)^2 + (average(3,i)-ave_g)^2;
        end
        for i=11:bg_num
            temp = (average(1,i)-ave_r)^2 + (average(2,i)-ave_b)^2 + (average(3,i)-ave_g)^2;
        ...
        r_f(x:x+pheight-1, y:y+pwidth-1) = r(x:x+pheight-1, y:y+pwidth-1) * 0.35 + bg_r(1:pheight, (image_index-1)*pwidth+1:image_index*pwidth) * 0.65;
        g_f(x:x+pheight-1, y:y+pwidth-1) = g(x:x+pheight-1, y:y+pwidth-1) * 0.35 + bg_g(1:pheight, (image_index-1)*pwidth+1:image_index*pwidth) * 0.65;
        b_f(x:x+pheight-1, y:y+pwidth-1) = b(x:x+pheight-1, y:y+pwidth-1) * 0.35 + bg_b(1:pheight, (image_index-1)*pwidth+1:image_index*pwidth) * 0.65;
    end
end
